  ...
  home = {
    ...
    sessionVariables = {
      ...
      IMDB_RENAME_TEMPLATE =
        "{{ if episode -}} tv/{series.primaryTitle}/Season {episode.seasonNumber | leading_zero}/{series.primaryTitle} - s{episode.seasonNumber | leading_zero}e{episode.episodeNumber | leading_zero} - {title.primaryTitle} {{- else -}} movies/{title.primaryTitle} ({title.startYear}) {{- endif }}";
    };
  };
